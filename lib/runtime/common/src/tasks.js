...
const getTaskAsync = async (options) => {
  return Common.Dynamo.getItem({
    ...
    .promise()
    .then((d) => (d?.Item?.status?.S ? JSON.parse(d.Item.status.S) : undefined));
};

const getTask = (options, cb) => {
  return getTaskAsync(options)
    .then((d) => cb(null, d))
    .catch(cb);
};

const updateTaskStatusAsync = async (newStatus) => {
  return getTaskAsync(newStatus).then((oldStatus) => {
    let updatedStatus = {
      ...newStatus,
    ...
    return Common.Dynamo.putItem({
      ...
      .promise()
      .then((d) => updatedStatus);
  });
};

const updateTaskStatus = (newStatus, cb) => {
  return updateTaskStatusAsync(newStatus)
    .then((d) => cb(null, d))
    .catch(cb);
};
...
const scheduleTaskAsync = async (taskConfig, task) => {
  ...
  return Common.SQS.sendMessage(params)
    .promise()
    .then(() => status);
};

const scheduleTask = (taskConfig, task, cb) => {
  return scheduleTaskAsync(taskConfig, task)
    .then((d) => cb(null, d))
    .catch(cb);
};
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        const results = await Promise.all(
          [url, taskConfig.queue.delayedUrl].map((QueueUrl) =>
