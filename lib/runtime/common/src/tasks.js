...
const updateTaskStatus = (newStatus, cb) => {
  return getTask(newStatus, (e, oldStatus) => {
    ...
    if (oldStatus?.notBefore) {
      newStatus.notBefore = oldStatus.notBefore;
    }
...
const getDelay = (ctx, total) => {
  const now = Date.now();
  const notBefore = +ctx.headers?.[NotBeforeHeader] * 1000 || 0;
  const delta = Math.floor((notBefore - now) / 1000);
  const delaySeconds = now >= notBefore ? undefined : total ? delta : Math.min(900, delta);
  return delaySeconds;
};
  ...
  return updateTaskStatus(
    {
      ...
      taskId: task.taskId,
      status: 'pending',
      ...(isDelayed ? { notBefore: new Date(+task.ctx.headers[NotBeforeHeader] * 1000).toISOString() } : {}),
    },
    (e, status) => {
      if (e) return cb(e);
