...
exports.loadLogging = () => {
  return async (req, res, next) => {
    if (Common.realtime_logs_enabled && exports.is_logging_enabled(req.params)) {
      const perm = { action: Constants.Permissions.logFunction, resource: getLogUrl(params) };
      ...
      if (req.functionSummary[Tags.get_compute_tag_key('permissions')]) {
        req.functionSummary[Tags.get_compute_tag_key('permissions')].allow.append(perm);
      } else {
...
exports.addLogging = (keyStore) => {
  return async (req, res, next) => {
    if (Common.realtime_logs_enabled && exports.is_logging_enabled(req.params)) {
      ...
      req.params.logs = { token: req.params.functionAccessToken, path: `/v1/${logUrl}`, host, protocol };
    }
    next();
  };
};
