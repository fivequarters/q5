...
const addLogging = (keyStore) => {
  return async (req, res, next) => {
    ...
    req.params.logs = await createLoggingCtx(keyStore, req.params, protocol, host);

    return next();
  };
};

const createLoggingCtx = async (keyStore, params, protocol, host) => {
  if (!Common.realtime_logs_enabled || !is_logging_enabled(params)) {
    return undefined;
  }
  ...
  const token = await mintJwtForPermissions(keyStore, params, permission);
