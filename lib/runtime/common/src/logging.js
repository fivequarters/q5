...
exports.loadLogging = () => {
  return async (req, res, next) => {
    if (req.functionSummary.permissions) {
      req.functionSummary.permissions.allow.append({ action: Constants.Permissions.logFunction, resource: logUrl });
    }
    next();
  };
};
  ...
  return async (req, res, next) => {
    ...
    if (Common.realtime_logs_enabled && exports.is_logging_enabled(req.params)) {
      const logUrl =
        ...
        `/boundary/${req.params.boundaryId}` +
        `/function/${req.params.functionId}` +
        `/log`;

      if (req.functionSummary.permissions) {
        req.params.logs = { token: req.headers.authorization, path: `/v1/${logUrl}`, host, protocol };
      } else {
        const token = await keyStore.signJwt({
          sub: Constants.makeFunctionSub(req.params, 'log'),
          [Constants.JWT_PERMISSION_CLAIM]: { allow: [{ action: 'function:post-logs', resource: logUrl }] },
        });
        req.params.logs = { token, path: `/v1/${logUrl}`, host, protocol };
      }
    }
    next();
  };
