...
const getLogPermission = (params) => ({
  action: Constants.Permissions.logFunction,
  resource: getLogUrl(params),
});

const addLogPermission = (params, functionSummary) => {
  const perm = getLogPermission(params);
  ...
  if (!functionSummary[Tags.get_compute_tag_key('permissions')]) {
    functionSummary[Tags.get_compute_tag_key('permissions')] = { allow: [perm] };
  } else if (!functionSummary[Tags.get_compute_tag_key('permissions')].allow) {
    functionSummary[Tags.get_compute_tag_key('permissions')].allow = [perm];
  } else {
    functionSummary[Tags.get_compute_tag_key('permissions')].allow.push(perm);
  }
};
...
const loadLogging = () => {
  return (req, res, next) => {
    if (!Common.realtime_logs_enabled || !is_logging_enabled(req.params)) {
      return next();
    }

    addLogPermission(req.params, req.functionSummary);

    return next();
  };
};

const addLogging = (keyStore) => {
  return async (req, res, next) => {
    ...
    req.params.logs = {
      token: req.params.functionAccessToken,
      path: `/v1${getLogUrl(req.params)}`,
      host,
      protocol,
    };
...
export { pollOnce, is_logging_enabled, getLogUrl, getLogPermission, addLogPermission, loadLogging, addLogging };
