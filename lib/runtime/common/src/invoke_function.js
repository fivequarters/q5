...
const invoke_function_handler = (req, next, e, r) => {
  ...
  if (e) {
    if (e.mode !== 'redirect' && e.code !== 'TooManyRequestsException') {
    ...
    } else if (e.code === 'TooManyPendingTasks' || e.code === 'ServiceException') {
      return next(create_error(429), null, meta);
    } else {
...
const print_error = (p, e) => {
  ...
  console.log(
    `INVOKE WARN: ${p.subscriptionId}/${p.boundaryId}/${p.functionId}: ${e.code} ${e.message} stacktrace: ${
