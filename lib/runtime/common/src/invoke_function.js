...
exports.invoke_function = (req, next) => {
  ...
  const timeout = (req.functionSummary['compute.timeout'] || 30) * 1000;
  ...
  return invoke_function_core(options, req.params.version, timeout, cb);
};

const makeShouldRetry = (timeout = 30000, initialDelay = 500, startTime = Date.now()) => {
  let attemptCount = 0;
  const shouldRetry = async (attemptCountOverride) => {
    const remainingTime = timeout - (Date.now() - startTime);
    const backoffTime = Math.pow(2, attemptCountOverride || attemptCount) * initialDelay;
    if (backoffTime > remainingTime) {
      return false;
    }

    attemptCount = (attemptCountOverride || attemptCount) + 1;
...
const invoke_function_http = async (options, redirectUrl, redirectHostname, timeout, cb) => {
  ...
  const shouldRetry = makeShouldRetry(timeout, 500, startTime);
...
const invoke_function_core = async (options, version, timeout, cb) => {
  let invoke_params = {
  ...
  const shouldRetry = makeShouldRetry(timeout);

  let err;
  do {
    try {
      const result = await Common.Lambda.invoke(invoke_params).promise();
...
const invoke_function_handler = (req, next, e, r) => {
  ...
  if (e) {
    ...
    } else if (e.code === 'LambdaServiceError') {
      return next(create_error(522), null, meta);
    } else {
