...
const MaxTaskRetryAfterSeconds = 60;
...
const schedule_function_core = async (req, ctx, cb) => {
  ...
  const delaySeconds = getDelay(req, true);
  if (delaySeconds > MaxDelaySeconds) {
    return cb({
      code: 'TooLargeDelaySeconds',
      message: `The ${NotBeforeHeader} header cannot specify EPOCH time more than ${MaxDelaySeconds} seconds in the future`,
    });
  }
  let isDone = false;
  ...
  try {
    ...
    return scheduleTask(taskConfiguration, task, (e, d) => {
      return e
        ? cb(e)
        : cb(null, {
            StatusCode: 202,
            Payload: {
              status: 202,
              headers: {
                location: `${Constants.get_function_management_endpoint(
                  req,
                  ctx.accountId,
                  ctx.subscriptionId,
                  ctx.boundaryId,
