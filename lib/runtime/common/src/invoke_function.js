...
exports.invoke_function = (req, next) => {
  ...
  return invoke_function_core(options, invoke_function_handler);
};
...
const invoke_function_handler = (e, r) => {
  ...
  try {
    ...
    if (r.LogResult) {
      meta = {...parse_lambda_logs(r.LogResult), ...meta};
    }
...
const parse_lambda_logs = (logResult) => {
  ...
  return { log, metrics: { duration, memory } };
}
