...
exports.dispatch_event = (e) => {
  ...
  // Reduce any security tokens present to their parsed value.
  if (ev.request.params.functionAccessToken) {
    ev.request.params.functionAccessToken =
      decodeJwt(ev.request.params.functionAccessToken, false, true) || 'INVALID_JWT';
  }

  if (ev.request.headers.authorization) {
    ev.request.headers.authorization = decodeJwt(ev.request.headers.authorization, false, true) || 'INVALID_JWT';
  }
