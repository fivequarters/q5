...
const { convertUrlToAttributes } = require('./attribute');
...
exports.dispatch_event = (e) => {
  ...
  const attributes = convertUrlToAttributes(`${Constants.API_PUBLIC_ENDPOINT}${e.request.url}`);

  // Now include all of the Fusebit-internal spans
  e.functionSpans.forEach((span) => convertUrlToAttributes(span.url, attributes));

  if (ev.fusebit.modality === 'execution') {
    // Extract out internal calls from the returned spans and add them as attributes to the summary log
    // message.
  ...
  if (process.env.GRAFANA_ENDPOINT) {
    ...
    // Write to loki
    grafana.publishLogs({ ...ev.fusebit, traceId: ev.traceId, spanId: ev.spanId }, attributes, e.functionLogs);
  }
