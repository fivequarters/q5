...
const transform = async payload => {
  ...
  for (let logEvent of payload.logEvents) {
    ...
    source.request.params.accountId = subscriptionAccountCache[source.request.params.subscriptionId];

    if (!source.request.params.accountId) {
      subscriptionAccountCache = await subscriptions.loadSubscriptions();
      source.request.params.accountId = subscriptionAccountCache[source.request.params.subscriptionId];
      if (!source.request.params.accountId) {
        console.log(`Unable to find account for ${source.request.params.subscriptionId}: ${JSON.stringify(source)}`);
      }
    }
...
function buildRequest(endpoint, body) {
  ...
  var request = {
    ...
    headers: {
      ...
      'X-Amz-Date': datetime,
      //            'Authorization': 'Basic ' + new Buffer('user:rhp3hy2}p26iKk3^tAMc?T~w').toString('base64')
    },
  };
  ...
  request.headers.Authorization = [
    ...
    //        'Basic ' + new Buffer('user:rhp3hy2}p26iKk3^tAMc?T~w').toString('base64')
  ].join(', ');
