...
export function patch_function_tag(options, key, value, cb) {
  const params = {
  ...
  if (value == undefined) {
    params.UpdateExpression = 'REMOVE #TK';
  } else {
    params.ExpressionAttributeValues = {
      ':tv': to_dynamo(value),
    };
    params.UpdateExpression = 'SET #TK = :tv';
  }
  (options.dynamo || dynamo).updateItem(params, cb);
}
...
export function from_dynamo(value) {
  if ('NULL' in value) {
    return null;
  }
  if ('S' in value) {
