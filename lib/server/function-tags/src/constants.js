...
export function convert_spec_to_tags(spec) {
  ...
  if (spec.security) {
    ...
    if (spec.security.authorization) {
      tags[get_security_tag_key('authorization')] = JSON.stringify(spec.security.authorization);
    }
...
const structurize = (tags) => {
  ...
  for (const [tag, value] of Object.entries(tags)) {
    ...
    // Use | as a deliminator to support arrays of values.
    lastW[lastK] = typeof value === 'string' ? (value.indexOf('|') > -1 ? value.split('|') : value) : value;
  }
...
const mPerms = (perms, view) => {
  ...
  for (const perm of perms) {
    const permStache = mustache.render(perm.resource, view);
    result = result.concat(
      permStache
        .trim()
        .split('\n') // Support list extrapolation for a particular action.
        .map((resource) => ({ action: perm.action, resource }))
    );
  }
