  ...
  if (spec.security) {
    ...
    if (spec.security.functionPermissions) {
      tags[get_security_tag_key('permissions')] = JSON.stringify({
        allow: mustachePermissions(spec.security.functionPermissions.allow, structuredTags),
      });
    }

    // Collect the authorization
    if (spec.security.authorization) {
      tags[get_security_tag_key('authorization')] = JSON.stringify(
        mustachePermissions(spec.security.authorization, structuredTags)
      );
    }
  } else {
