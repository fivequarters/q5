...
export function convert_spec_to_tags(spec) {
  ...
  if (spec.functionPermissions) {
    tags[get_compute_tag_key('permissions')] = JSON.stringify(spec.functionPermissions);
  }

  // Collect the permissions
  if (spec.authorizations) {
    tags[get_compute_tag_key('authorizations')] = JSON.stringify(spec.authorizations);
  }

  // Collect the authentication
  tags[get_compute_tag_key('authentication')] = spec.authentication || 'none';
