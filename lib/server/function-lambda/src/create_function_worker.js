...
const maxFunctionUpdateWait = 60 * 1000;
...
function update_user_function(ctx, cb) {
  ...
  Common.Lambda.updateFunctionConfiguration(update_function_config_params, async (e, d) => {
    ...
    // publishVersion returns a 409, which prevents the new function from being executed.
    if (d.LastUpdateStatus == 'InProgress') {
      try {
        ...
        if (lastUpdateStatus !== 'Successful') {
          return cb(new Error(`Failed to settle function with status ${lastUpdateStatus}`));
        }
      } catch (e) {
        return cb(e);
      }
...
async function waitForFunction(functionName, maxWait) {
  ...
  while (d.Configuration.LastUpdateStatus == 'InProgress') {
    // Delay pollInterval before trying again.
