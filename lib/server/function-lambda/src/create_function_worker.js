...
async function waitForFunctionAndCheckForError(options, functionType, functionName) {
  const state = await waitForFunction(functionName, maxFunctionCreateWait);
    ...
    return Common.Lambda.invoke(builder_invoke_params, async (e, d) => {
      if (e) {
        if (e.code !== 'ResourceNotFoundException' || is_final) {
          console.log(
            `${ctx?.options?.accountId}/${ctx?.options?.subscriptionId}/${ctx?.options?.boundaryId}/${ctx?.options?.functionId} LOGGING ERROR (module_compile_pass, invoke): ${e.code}, ${name}:${ctx.options.internal.resolved_dependencies[name].version}`,
            e
          );
        }
        if (e.code === 'ResourceNotFoundException' && !is_final) {
