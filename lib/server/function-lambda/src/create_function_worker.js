...
export function create_function_worker(status, cb) {
  Assert.ok(status);
...
async function create_npmrc(ctx) {
  const spec = { accountId: ctx.options.accountId, subscriptionId: ctx.options.subscriptionId, registryId: 'default' };
  const reg = AWSRegistry.create(spec);
  const config = await reg.configGet();
  const npmUrl =
    process.env.API_SERVER +
    `/v1/account/${spec.accountId}/subscription/${spec.subscriptionId}/registry/${spec.registryId}/npm/`;

  const protoUrl = url.replace(/^http[s]?:/, '');
  return [`${protoUrl}:_authToken=${ctx.authentication}`, reg.scopes.map((scope) => `${scope}:registry=${npmUrl}`)];
}
