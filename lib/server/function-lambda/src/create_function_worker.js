  ...
  return Async.series(
    ...
    (e) => {
      if (e) {
        /* XXX*/
        console.log(
          `${ctx?.options?.accountId}/${ctx?.options?.subscriptionId}/${ctx?.options?.boundaryId}/${
            ctx?.options?.functionId
          } LOGGING ERROR (create_function_worker): ${JSON.stringify(status)}`,
          e
        );
        /* XXX*/
...
function update_user_function(ctx, cb) {
  ...
  Common.Lambda.updateFunctionConfiguration(update_function_config_params, async (e, d) => {
    ...
    if (d.LastUpdateStatus == 'InProgress') {
      const lastUpdateStatus = await waitForFunction(functionName, maxFunctionUpdateWait);
      if (lastUpdateStatus !== 'Successful') {
        return cb(new Error(`Failed to settle function ${functionName}: ${lastUpdateStatus}`));
      }
    }
    ...
    return Common.Lambda.updateFunctionCode(update_function_params, async (e, d) => {
      ...
      if (d.State != 'Active') {
        const lastUpdateStatus = await waitForFunction(newVersionName, maxFunctionUpdateWait);
        if (lastUpdateStatus !== 'Successful') {
          return cb(new Error(`Failed to settle function ${newVersionName}: ${lastUpdateStatus}`));
        }
      }
...
async function waitForFunction(functionName, maxWait, options) {
  ...
  try {
    ...
    /* XXX*/
    if (options?.create_module_builder_logs) {
      options.create_module_builder_logs.push(JSON.stringify(d));
    }
    /* XXX*/
    ...
    // Update Status is no longer InProgress and State is not Pending; report whatever occured.
    return normalizeFunctionWaitState(d.Configuration);
  } catch (e) {
    /* XXX*/
    if (options?.create_module_builder_logs) {
      options.create_module_builder_logs.push(`waitForFunction error ${e.code}`);
    }
    /* XXX*/
...
function update_user_function_config(ctx, cb) {
  ...
  return Common.Lambda.updateFunctionConfiguration(update_function_params, async (e, d) => {
    ...
    return Common.Lambda.publishVersion({ FunctionName: functionName }, async (err, d) => {
      ...
      console.log(`XXX XXX publishVersion new code: ${d.State}`);
  ...
  return Async.series(
    ...
    (e) => {
      if (e) {
        /* XXX*/
        console.log(
          `${ctx?.options?.accountId}/${ctx?.options?.subscriptionId}/${ctx?.options?.boundaryId}/${ctx?.options?.functionId} LOGGING ERROR (compile_missing_dependency): ${e.code}, ${name}:${ctx?.options?.internal?.resolved_dependencies[name]?.version}`,
          e
        );
        /* XXX*/
        return cb(e);
    ...
    return Async.series(
      ...
      (e) => {
        if (e) {
          /* XXX*/
          console.log(
            `${ctx?.options?.accountId}/${ctx?.options?.subscriptionId}/${ctx?.options?.boundaryId}/${ctx?.options?.functionId} LOGGING ERROR (create_module_builder_then_compile): ${e.code}, ${name}:${ctx?.options?.internal?.resolved_dependencies[name]?.version}`,
            e
          );
          /* XXX*/
      ...
      if (e) {
        ...
        if (e.code !== 'ResourceNotFoundException' || is_final) {
          console.log(
            `${ctx?.options?.accountId}/${ctx?.options?.subscriptionId}/${ctx?.options?.boundaryId}/${
              ctx?.options?.functionId
            } LOGGING ERROR (module_compile_pass, invoke): ${e.code}, is_final: ${is_final}, ${name}:${
              ctx?.options?.internal?.resolved_dependencies[name]?.version
            }, ${JSON.stringify(ctx?.options?.create_module_builder_logs)}`,
            e
          );
        }
        /* XXX*/
...
function create_module_builder(name, ctx, cb) {
  ...
  return Common.Lambda.createFunction(create_builder_params, async (e, d) => {
    if (e) {
      /* XXX*/
      {
        // Theory: createFunction fails with a ResourceConflictException but the function is not yet ready, so
        // when it attempts to execute it, it comes back with resource not found.
        const logHeader = `${ctx?.options?.accountId}/${ctx?.options?.subscriptionId}/${ctx?.options?.boundaryId}/${ctx?.options?.functionId} LOGGING ERROR `;
        console.log(
          `${logHeader} (create_module_builder): ${e.code}, ${name}:${ctx?.options?.internal?.resolved_dependencies[name]?.version}`,
          e
        );
        (async () => {
          try {
            let d = await Common.Lambda.getFunction({ FunctionName: functionName }).promise();
            console.log(`${logHeader} (create_module_builder, function, data): ${JSON.stringify(d)}`);
          } catch (e) {
            console.log(`${logHeader} (create_module_builder, function, error): ${e.code}`);
          }
        })();
      }
      /* XXX*/
    ...
    /* XXX*/
    ctx.options.create_module_builder_logs = [JSON.stringify(d)];
    /* XXX*/
