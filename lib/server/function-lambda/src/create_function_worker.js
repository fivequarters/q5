...
export function create_function_worker(status, registry, authentication, cb) {
  Assert.ok(status);
  ...
  var ctx = { status, registry, authentication };
    ...
    (e) => {
      ...
      return Async.series(
        [
          ...
          (cb) => (e ? cb() : create_function_tags(ctx.options, ctx.options, cb)),
...
function compile_missing_dependency(name, ctx, cb) {
  ...
  console.log('XYXYX compile_missing_dependency');
  ...
  return Async.series(
    [
      ...
      (cb) => {
        console.log('YYYYYYYYY Next step');
        cb();
      },
  ...
  function create_module_builder_then_compile(delete_before_creating) {
    console.log('XXYXYX create_module_builder_then_compile');
    return Async.series(
      [
  ...
  function generate_npmrc(ctx, cb) {
    console.log('XXX XXX XXX calling npmrc');
    console.log(`ctx: ${Object.keys(ctx)}`);
    create_npmrc(ctx).then((npmrc) => {
      console.log(`XXX XXX XXX npmrc created \n${npmrc}`);
  ...
  function update_module_metadata_and_finish(build_error) {
    var metadata = ctx.options.internal.missing_dependencies[name] || {};
...
async function create_npmrc(ctx) {
  ...
  const npmUrl =
    'http://' +
    process.env.LOGS_HOST +
    `/v1/account/${spec.accountId}/subscription/${spec.subscriptionId}/registry/${spec.registryId}/npm/`;

  const protoUrl = npmUrl.replace(/^http[s]?:/, '');
  return [
    `${protoUrl}:_authToken=${ctx.authentication.token.replace(/^Bearer /, '')}`,
