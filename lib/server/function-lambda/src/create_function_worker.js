...
const { Common, is_logging_enabled, getLogUrl, getLogPermission } = require('@5qtrs/runtime-common');
...
const { mintJwtForPermissions } = require('@5qtrs/runas');
...
async function enqueue_imminent_cron(ctx, cb) {
  ...
  if (true || is_logging_enabled(ctx.options)) {
    const perm = getLogPermission(ctx.options);

    // Add the realtime logging permissions to the summary.
    if (!ctx.options.permissions) {
      ctx.options.permissions = { allow: [perm] };
    } else if (!ctx.options.permissions.allow) {
      ctx.options.permissions.allow = [perm];
    } else {
      ctx.options.permissions.allow.push(perm);
    }
  }

  // Mint a JWT, if necessary, and add it to the context.
  const functionAccessToken = await mintJwtForPermissions(
    ctx.authorization.keyStore,
    ctx.options,
    ctx.options.permissions,
    'cron'
  );
  ...
  if (true || is_logging_enabled(ctx.options)) {
    ...
    ctx.options.logs = {
      token: functionAccessToken,
      path: `/v1/${getLogUrl(ctx.options)}`,
      host: Constants.API_PUBLIC_ENDPOINT.replace(/http[s]?:\/\//i, ''),
      protocol: 'https',
    };
    console.log(`Enabling logging ${JSON.stringify(ctx.options.logs)}`);
  }
