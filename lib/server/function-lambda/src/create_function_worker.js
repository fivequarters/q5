...
// TODO: workaround the lambda state change issue from 11/23/2021
const AWSWorkaround = `flag(aws:states:opt-out) `;
...
function create_new_user_function(ctx, cb) {
  ...
  let create_function_params = {
    ...
    Description: AWSWorkaround + Constants.get_user_function_description(ctx.options),
    Runtime: ctx.options.compute.runtime,
  ...
  return Common.Lambda.createFunction(create_function_params, (e, d) => {
    if (e) {
...
async function waitForFunction(functionName, maxWait) {
  ...
  try {
    ...
    while (d.Configuration.LastUpdateStatus == 'InProgress') {
      ...
      if (Date.now() > quitAfter) {
        return d.Configuration.LastUpdateStatus;
      }

      d = await Common.Lambda.getFunction({ FunctionName: functionName }).promise();
    }

    // Status is no longer InProgress; report whatever occured.
    return d.Configuration.LastUpdateStatus;
  } catch (e) {
...
function create_function_builder(options, cb) {
  let create_builder_params = {
    FunctionName: Constants.get_function_builder_name(options),
    Description: AWSWorkaround + Constants.get_function_builder_description(options),
...
function create_module_builder(name, ctx, cb) {
  let create_builder_params = {
    ...
    Description:
      AWSWorkaround +
      Constants.get_module_builder_description(ctx, name, ctx.options.internal.resolved_dependencies[name]),
    Runtime: ctx.options.compute.runtime,
