...
function get_builder_name(options) {
  return Crypto.createHash('sha1')
    .update(get_builder_description(options))
    .digest('hex');
}
...
function create_builder(options, cb) {
  return Async.waterfall([cb => ensure_builder_zip(cb), (zip, cb) => create_builder_function(zip, options, cb)], cb);
}

function create_module_builder(name, ctx, cb) {
  return Async.waterfall(
    [cb => ensure_module_builder_zip(cb), (zip, cb) => create_module_builder_function(zip, name, ctx, cb)],
    cb
  );
}
...
function ensure_builder_zip(cb) {
  ...
  return zip.generateAsync({ type: 'nodebuffer' }).then(c => {
    builder_zip = c;
    return cb(null, c);
  });
}
...
function ensure_module_builder_zip(cb) {
  ...
  zip.file(
    'module_builder.js',
    Fs.readFileSync(Path.join(__dirname, '../lambda/module_builder.js'), { encoding: 'utf8' })
  );
  zip.file('zip.py', Fs.readFileSync(Path.join(__dirname, '../lambda/zip.py'), { encoding: 'utf8' }));
  return zip.generateAsync({ type: 'nodebuffer' }).then(c => {
