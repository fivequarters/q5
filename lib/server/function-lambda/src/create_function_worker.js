...
export function create_function_worker(status, registry, authorization, cb) {
  ...
  return Async.series(
    [
      ...
      //   cb();
      // },
      (cb) => update_user_function_enable(ctx, cb), // enable or disable function
    ],
    (e) => {
      if (e) {
...
export function finalize_function_status(ctx, cb) {
  return Async.series(
    [
      (cb) => save_function_spec(ctx.options, cb),
      (cb) => create_function_tags(ctx.options, ctx.options, cb),
...
export function update_user_function_enable(ctx, cb) {
  if (ctx.options.enable === false) {
    ...
    return Common.Lambda.deleteFunctionConcurrency({ FunctionName: Constants.get_user_function_name(ctx.options) }, cb);
  }
}
...
async function create_npmrc(ctx) {
  ...
  return [
    `${protoUrl}:_authToken=${ctx.authorization.token.replace(/^Bearer /, '')}`,
