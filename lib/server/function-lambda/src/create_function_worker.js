...
export function create_function_worker(status, registry, authorization, cb) {
  ...
  return Async.series(
    [
      ...
      //   cb();
      // },
      (cb) => update_user_function_enable(ctx.options, cb), // enable or disable function
    ],
    (e) => {
      if (e) {
        console.log('XXX in final area of create_function_worker');
      ...
      console.log(`XXX starting async series ${e ? 'error' : ''}`);
      return Async.series(
        [
        ...
        (e) => {
          console.log(`XXX finished async series ${e ? 'error' : ''}`);
  ...
  function transition_state(new_state) {
    console.log(`transition_state ${new_state}`);
...
export function finalize_function_status(ctx, cb) {
  console.log(`finalize_function_status`);
  return Async.parallel(
    [
      (cb) => save_function_spec(ctx, cb),
      (cb) => create_function_tags(ctx, ctx, cb),
      (cb) => update_user_function_enable(ctx, cb),
    ],
    (e) => {
      cb(e);
    }
...
function save_function_build_status(ctx, cb) {
  console.log(`save_function_build_status`);
  if (ctx.options && ctx.options.internal && ctx.options.internal.build_plan !== 'full_build') {
    console.log(`save_function_build_status ${ctx.options.internal.build_plan}`);
...
function delete_build_request(options, cb) {
  console.log(`delete_build_request ${Constants.get_user_function_build_request_key(options)}`);
...
export function update_user_function_enable(ctx, cb) {
  if (ctx.enable === false) {
    return Common.Lambda.putFunctionConcurrency(
      {
        FunctionName: Constants.get_user_function_name(ctx),
        ReservedConcurrentExecutions: 0,
      },
      cb
    );
  } else {
    return Common.Lambda.deleteFunctionConcurrency({ FunctionName: Constants.get_user_function_name(ctx) }, cb);
  }
}
...
function get_module_builder_description(ctx, name) {
  return `module-builder:${ctx.options.accountId}:${ctx.options.subscriptionId}:default:${ctx.options.compute.runtime}:${name}:${builder_version}`;
}
...
function get_module_builder_name(ctx, name) {
  return Crypto.createHash('sha1').update(get_module_builder_description(ctx, name)).digest('hex');
}
...
function compile_missing_dependency(name, ctx, outerCb) {
  const cb = (e) => {
    console.log(`Final callback: ${JSON.stringify(e.FunctionError)}`);
    outerCb(e);
  };
  ...
  let builder_invoke_params = {
    FunctionName: get_module_builder_name(ctx, name),
    Payload: {
      name,
  ...
  console.log(`compile_missing_dependency: ${name}:${ctx.options.internal.resolved_dependencies[name].version}`);
  return Async.series(
    [
    ...
    (e) => {
      console.log(`XXX compile_missing_dependency error: ${e}`);
      if (e) return cb(e);
    }
  );
  ...
  function module_compile_pass(is_final) {
    return Common.Lambda.invoke(builder_invoke_params, (e, d) => {
      console.log(`module_compile_pass ${e} ${JSON.stringify(d.FunctionError, null, 2)}`);
      if (e) {
  ...
  function update_module_metadata_and_finish(build_error) {
    console.log(`u_m_m_a_f ${build_error ? build_error.length : 0}`);
    ...
    console.log(
      `writing metadata ${Constants.get_module_metadata_key(
        ctx.options.compute.runtime,
        name,
        ctx.options.internal.resolved_dependencies[name]
    ...
    return Common.S3.putObject(
      ...
      (e, d) => {
        console.log(`putObject Completed: e: ${JSON.stringify(e)} ${JSON.stringify(metadata.status)}`);
        if (e) return cb(e);
...
function compile_deployment_package(ctx, cb) {
  ...
  console.log(`compile_dep_pkg: ${JSON.stringify(dependencies)}`);
...
function create_module_builder(name, ctx, cb) {
  let create_builder_params = {
    FunctionName: get_module_builder_name(ctx, name),
    Description: get_module_builder_description(ctx, name),
    Runtime: ctx.options.compute.runtime,
...
async function create_npmrc(ctx) {
  ...
  const npmUrl = process.env.API_SERVER + `/v1/account/${spec.accountId}/registry/${spec.registryId}/npm/`;
