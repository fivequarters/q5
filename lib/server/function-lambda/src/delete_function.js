...
const { delete_function_tags } = require('./manage_tags');
...
function delete_function_core(options, cb) {
  ...
  return Async.series(
    [
      ...
      (cb) => delete_metadata_tags(options, cb),
...
function delete_metadata_tags(options, cb) {
  return Common.S3.getObject(
    ...
    (e, d) => {
      ...
      delete_function_tags(options, d.Body, cb);
    }
  );
}
