...
function intersect_results(e, d, cb) {
  let result = {};
  ...
  for (let i = 1; i < d.length && Object.keys(result).length > 0; i++) {
    const newResult = {};
    ...
    d[i].items.forEach((f) => {
      const m = result[f.location];
      if (m) {
        newResult[f.location] = m;
      }
    });
    result = newResult;
  }

  let nexts = d.map((i) => i.next);

  // Don't bother returning an empty next set
  if (nexts.filter((n) => n).length == 0) {
    nexts = undefined;
  }
