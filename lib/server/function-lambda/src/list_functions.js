...
function list_functions_core(options, cb) {
  ...
  function search_functions(cb) {
    let category, options, key, value, continuationToken;

    let params = {
      TableName: keyValueTableName,
      ProjectionExpression: 'tagKey, tagValue, boundaryId, functionId',
      Limit: 100,
      ExpressionAttributeValues: {
        ':searchCategory': { S: category },
        ':sortKey': { S: 'key' },
        ':sortKeyPrefix': { S: create_boundary_search_key(options, key, value) },
        ':tagKey': { S: key },
        ':tagValue': { S: value },
      },
      KeyConditionExpression: `category = :searchCategory AND begins_with(:sortKey, :sortKeyPrefix)`,
      FilterExpression: `tagKey = :tagKey AND tagValue = :tagValue`,
