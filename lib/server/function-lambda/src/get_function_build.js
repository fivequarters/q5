...
const { create_build_error_response } = require('./build_status');

export function get_function_build(req, res, next) {
  return get_function_build_core(req.params, (e, r) => {
    if (e) {
      if (e.code === 'NoSuchKey') {
    ...
    switch (r.status) {
      ...
      case 'building':
        ...
        break;
      case 'failed':
        r = create_build_error_response(r);
        res.status(r.status);
        break;
      default:
        res.status(500);
...
function get_function_build_core(options, cb) {
  Assert.ok(options);
  ...
  console.log(`get_function_build_core ${Constants.get_user_function_build_status_key(options)}`);
  return Common.S3.getObject(
    {
      Bucket: process.env.AWS_S3_BUCKET,
      Key: Constants.get_user_function_build_status_key(options),
    },
    (e, d) => {
      if (e) return cb(e);
      try {
        d.Body = JSON.parse(d.Body.toString('utf8'));
        console.log(`get_function_build_core ${JSON.stringify(d.Body)}`);
      } catch (e) {
        return cb(e);
      }
