...
module.exports = function lambda_put_function(req, res, next) {
  ...
  if (req.body.schedule) {
    ...
    } else if (req.body.schedule.timezone) {
      return next(
        create_error(
          400,
          "The 'schedule.timezone' body parameter can only be specified if the 'schedule.cron' body paramter is also specified"
        )
      );
    }
