...
async function resolve_dependencies_from_fusebit(ctx, cb) {
  ...
  return Async.eachLimit(
    /* XXX don't filter, find first that matches */
    Object.keys(ctx.options.internal.pending_dependencies).filter(
      (n) => cfg.scopes.filter((s) => n.startsWith(s)).length > 0
    ),
  ...
  async function resolve_one_dependency(ctx, name, cb) {
    ...
    try {
      ...
      console.log(`XXX Found version: '${version}'`);
      if (!version) {
  ...
  function check_one_dependency_present_in_s3(ctx, name, cb) {
    return Common.S3.getObject(
      ...
      (e, d) => {
        ...
        /* XXX XXX XXX */
        if (false && d.Body.backoff && d.Body.backoff > Date.now()) {
...
function run_build(ctx, cb) {
  ...
  if (ctx.options.internal.build_plan === 'full_build') {
    process.nextTick(() => Worker(ctx.build_status, ctx.registry, ctx.authentication));
    ...
    return Worker(ctx.build_status, ctx.registry, ctx.authentication, (e, d) =>
