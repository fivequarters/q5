      ...
      new Promise((resolve, reject) => {
        ...
        cdns.forEach((cdn) =>
          promise.catch((e) => {
            ...
            let startTime = process.hrtime.bigint() / BigInt(1000);
            return Superagent.get(cdn)
              .timeout({ deadline: remainingTime, response: Math.min(response, remainingTime) })
              .then((res) => {
              ...
              .catch(() => {
                const endTime = process.hrtime.bigint() / BigInt(1000);
        ...
        promise.then(modifyCtx).then(resolve).catch(reject);
      });

    return Promise.race([timeoutPromise(), cdnPromise()]).catch((err) => {
      return cb(
        create_error(
          400,
          `Unable to fully resolve module version for ${name}@${ctx.options.internal.pending_dependencies[name]} from either ${jsdelvr} or ${unpkg}: ${err.message}`
        )
      );
    });
  }
