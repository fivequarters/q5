...
function resolve_settings(ctx, cb) {
  ...
  if (
    resolve_settings_core(ctx, 'compute', 'computeSettings', cb) ||
    resolve_settings_core(ctx, 'configuration', 'applicationSettings', cb) ||
    resolve_settings_core(ctx, 'schedule', 'cronSettings', cb)
  ) {
    return;
  }
  ...
  const updatedWithDefaults = update(ctx.options.metadata.fusebit.computeSettings, { values: ctx.options.compute });
...
function resolve_settings_core(ctx, key, metadataKey, cb) {
  const previousRaw =
    ctx.options.internal.existing.metadata.fusebit[metadataKey] || serialize(ctx.options.internal.existing[key] || {});

  // PreviousRaw will only be undefined if this is a new function
  // In this case, old clients will ensure that the structured and serialized
  // data agree, so it is fine to treat the current serialized data as the previous.
  // New clients will only send structured data if it is the source of truth
  // and the serailzed data is unchanged, therefore we can treat current serialized
  // data as the previous data for new functions.
  if (previousRaw === undefined) {
    previousRaw = ctx.options.metadata.fusebit[metadataKey] || '';
  }

  try {
    const updated = update(previousRaw, {
      values: ctx.options[key],
      serialized: ctx.options.metadata.fusebit[metadataKey],
    });
          ...
          return cb(
            new Error(
              `Unable to determine availability of module ${name}@${
                ctx.options.internal.resolved_dependencies[name]
              } for runtime ${ctx.options.compute.runtime}: ${e.error}`
            )
          );
        }
        try {
