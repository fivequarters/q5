...
function determine_dependencies_available(ctx, cb) {
  ...
  if (
    !ctx.options.internal.forceRebuild &&
    ...
    ctx.options.internal.pending_dependencies = { ...ctx.options.internal.dependencies };
    ...
    return Async.series(
      [
        ...
        (cb) => {
          if (ctx.options.internal.forceRebuild) {
            ctx.options.internal.missing_dependencies = ctx.options.internal.resolved_dependencies;
            cb();
          } else {
