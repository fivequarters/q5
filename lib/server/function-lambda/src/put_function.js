...
function compute_build_plan(ctx, cb) {
  ...
  ctx.options.internal.hashes.authorization = hash_object({
...
function compute_build_plan_core(ctx, cb) {
  ...
  if (hashes.authorization !== existingHashes.authorization) {
    // Only authorization has changed, no need to touch the function.
    return set_build('metadata_update');
  }
