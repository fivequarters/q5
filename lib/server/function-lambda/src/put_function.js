...
function resolve_settings(ctx, cb) {
  ...
  // Ignore all properties unrelated to cron
  ctx.options.schedule = ctx.options.schedule || {};
  let normalizedSchedule = {};
  if (ctx.options.schedule.cron) normalizedSchedule.cron = ctx.options.schedule.cron;
  if (ctx.options.schedule.timezone) normalizedSchedule.timezone = ctx.options.schedule.timezone;
  ctx.options.schedule = normalizedSchedule;
