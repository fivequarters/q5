...
function determine_dependencies_available(ctx, cb) {
  ...
  console.log(
    `determine_dependencies_available: force: ${internal.forceRebuild} ${JSON.stringify(internal.dependencies)}`
  );
  if (
    ...
    return Async.series(
      [
        ...
        (cb) => {
          console.log(
            `determine_d_a: missing: ${JSON.stringify(internal.missing_dependencies)} resolve: ${JSON.stringify(
              internal.resolved_dependencies
            )}`
          );
...
function check_dependencies_present_in_s3(ctx, cb) {
  ...
  function check_one_dependency_present_in_s3(ctx, name, cb) {
    console.log(
      `check_in_s3 ${name} ${Constants.get_module_metadata_key(
