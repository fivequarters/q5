...
function refresh_options_for_plan(ctx, cb) {
  ...
  if (build_plan === 'full_build' || build_plan === 'partial_build' || build_plan === 'fast_override') {
...
function compute_build_plan(ctx, cb) {
  ...
  // 2. fast_override (sync+async): Update metadata with override information and start an async full_build.
  // 3. partial_build (sync): create deployment package of the function by pulling existing modules
  //    from S3 and replace or create the function
  // 4. configuration_update (sync): update configuration of existing Lambda function
  // 5. metadata_update (sync): update function spec in S3
  // 6. none (sync): no changes requested
...
function compute_build_plan_core(ctx, cb) {
  ...
  if (
    hashes.code !== existingHashes.code &&
    hashes.configuration === existingHashes.configuration &&
    hashes.serialized === existingHashes.serialized &&
    hashes.security === existingHashes.security &&
    hashes.fusebitEditor === existingHashes.fusebitEditor &&
    hashes.metadata === existingHashes.metadata &&
    ctx.options.internal.cron_plan === 'none'
  ) {
    // Code has changed and nothing else has; perform a fast-build override
    return set_build('fast_override');
  }
...
function run_build(ctx, cb) {
  ...
  if (ctx.options.internal.build_plan === 'fast_override') {
    // Update the metadata
    // XXX
    //  Modify convert_spec_to_tags to include the override
    //  Leave a note as to the build key
    //  Conditionally update as long as build key is either equal or absent, otherwise leave it alone, on
    //    final commit.
  }

  if (ctx.options.internal.build_plan === 'full_build' || ctx.options.internal.build_plan === 'fast_override') {
