...
async function getRuntimeLayers() {
  try {
    ...
    console.log('ERROR GETTING NDOE.JS RUNTIME LAYERS FROM CDN', e.stack || e.message || e);
  }
}
getRuntimeLayers();
...
export function custom_layers_health() {
  if (!custom_runtime_layers) {
    throw new Error('Unable to get custom AWS Lambda layers for Node.js from CDN');
  }
}
...
function resolve_settings(ctx, cb) {
  ...
  if (requestedStaticIp && process.env.GRAFANA_ENDPOINT) {
    return cb(create_error(400, 'Static IP feature is not supported for this Fusebit deployment.'));
  }
