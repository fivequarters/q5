...
function compute_build_plan(ctx, cb) {
  ...
  ctx.options.internal.hashes.canFork = hash_object({ canFork: ctx.options.canFork });
...
function compute_build_plan_core(ctx, cb) {
  ...
  if (hashes.canFork !== existingHashes.canFork) {
    // Only canFork has changed, no need to touch the function.
    return set_build('metadata_update');
  }
