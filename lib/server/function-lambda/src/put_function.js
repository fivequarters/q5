...
function compute_build_plan_core(ctx, cb) {
  ...
  if (Object.keys(ctx.options.internal.existing_metadata).length === 0) {
    set_new_function();
  }
...
function determine_dependencies_available(ctx, cb) {
  ...
  if (
    internal.new_metadata.runtime_hash === internal.existing_metadata.runtime_hash &&
    internal.new_metadata.dependencies_hash === internal.existing_metadata.dependencies_hash
  ) {
    internal.resolved_dependencies = internal.existing_function_spec.internal.resolved_dependencies || {};
    return cb();
  } else {
    ...
    (e, d) => {
      if (e) {
        if (e.code === 'NoSuchKey' || e.code === 'NotFound') {
          ctx.options.internal.existing_function_spec = { internal: {} };
          ctx.options.internal.existing_metadata = {};
          return cb();
        } else return cb(e);
      }
      try {
        ctx.options.internal.existing_function_spec = JSON.parse(d.Body.toString('utf8'));
        ctx.options.internal.existing_metadata =
          ctx.options.internal.existing_function_spec.internal.new_metadata || {};
      } catch (e) {
        return cb(e);
      }
      return cb();
