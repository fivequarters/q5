...
module.exports = function lambda_put_function(req, res, next) {
  ...
  if (req.body.schedule) {
    ...
    if (req.body.schedule.timezone && !Moment.tz.zone(req.body.schedule.timezone)) {
      return next(
        create_error(
          400,
  ...
  if (package_json && package_json.engines && typeof package_json.engines.node === 'string') {
    try {
      ...
      if (!options.lambda.runtime) throw new Error('Cannot find matching Node version');
    } catch (_) {
      return next(
        create_error(
          400,
...
module.exports.core = function lambda_put_function_core(options, cb) {
  ...
  options.lambda.memorySize = options.lambda.memorySize || +process.env.LAMBDA_USER_FUNCTION_MEMORY_SIZE || 128;
  options.lambda.timeout = options.lambda.timeout || +process.env.LAMBDBA_USER_FUNCTION_TIMEOUT || 30;
  options.lambda.staticIp = options.lambda.staticIp || undefined;
...
function compute_build_plan(ctx, cb) {
  // This will compute ctx.options.internal.metadata and set ctx.options.internal.build_plan as follows:
  ...
  ctx.options.internal.metadata = {};

  // Compute hashes
  ctx.options.internal.metadata.code_hash = hash_object(ctx.options.nodejs.files);
  ctx.options.internal.metadata.lambda_hash = hash_object(ctx.options.lambda);
  ctx.options.internal.metadata.runtime_hash = hash_object({ runtime: ctx.options.lambda.runtime });
  ctx.options.internal.metadata.dependencies_hash = hash_object(ctx.options.internal.dependencies);
  ctx.options.internal.metadata.configuration_hash = hash_object(ctx.options.configuration);
  ctx.options.internal.metadata.metadata_hash = hash_object(ctx.options.metadata);
  ctx.options.internal.metadata.schedule_hash = hash_object(ctx.options.schedule || {});
...
function compute_build_plan_core(ctx, cb) {
  ...
  if (Object.keys(ctx.options.internal.existing.internal.metadata).length === 0) {
    set_new_function();
  }

  // Determine CRON update plan
  if (ctx.options.internal.metadata.schedule_hash === ctx.options.internal.existing.internal.metadata.schedule_hash) {
    // CRON schedule has not changed or function is not a cron job
    set_cron('none');
  } else if (
    ctx.options.internal.metadata.schedule_hash === empty_hash &&
    !ctx.options.internal.existing.internal.metadata.schedule_hash
  ) {
    // Function is not a cron job
    set_cron('none');
  } else if (ctx.options.internal.metadata.schedule_hash === empty_hash) {
    // Function has its CRON status removed
    set_cron('cancel');
  } else if (
    ctx.options.internal.existing.internal.metadata.schedule_hash === empty_hash ||
    !ctx.options.internal.existing.internal.metadata.schedule_hash
  ) {
  ...
  if (ctx.options.internal.metadata.runtime_hash !== ctx.options.internal.existing.internal.metadata.runtime_hash) {
    // Runtime has changed, but all dependencies for the new runtime are available in S3.
    return set_build('partial_build');
  }

  if (
    ctx.options.internal.metadata.dependencies_hash !==
    ctx.options.internal.existing.internal.metadata.dependencies_hash
  ) {
    // Dependencies have changed, but all are available in S3.
    return set_build('partial_build');
  }

  if (ctx.options.internal.metadata.code_hash !== ctx.options.internal.existing.internal.metadata.code_hash) {
    // Code has changed, dependencies remain unchanged.
    return set_build('partial_build');
  }

  if (
    ctx.options.internal.metadata.configuration_hash !==
    ctx.options.internal.existing.internal.metadata.configuration_hash
  ) {
  ...
  if (ctx.options.internal.metadata.metadata_hash !== ctx.options.internal.existing.internal.metadata.metadata_hash) {
...
function determine_dependencies_available(ctx, cb) {
  ...
  if (
    internal.metadata.runtime_hash === internal.existing.internal.metadata.runtime_hash &&
    internal.metadata.dependencies_hash === internal.existing.internal.metadata.dependencies_hash
  ) {
    ...
    (e, d) => {
      if (e) {
        if (e.code === 'NoSuchKey' || e.code === 'NotFound') {
          ctx.options.internal.existing = { internal: { metadata: {} } };
          return cb();
        } else return cb(e);
      }
      try {
        ...
        ctx.options.internal.existing.internal.metadata =
          ctx.options.internal.existing.internal.metadata || ctx.options.internal.existing.internal.new_metadata || {};
