...
import { create_function_worker as Worker, finalize_function_status } from './create_function_worker';
...
function compute_build_plan(ctx, cb) {
  ...
  ctx.options.internal.hashes.functionPermissions = hash_object({
    functionPermissions: ctx.options.functionPermissions,
  });
  ctx.options.internal.hashes.authorizations = hash_object({
    authorizations: ctx.options.authorizations,
  });
...
function resolve_settings(ctx, cb) {
  ...
  if (!ctx.options.functionPermissions) {
    return cb();
  }

  // Validate permissions are a subset of the current authorized entity
  ctx.authorization.agent
    .checkPermissionSubset(ctx.options.functionPermissions)
    .then(() => cb())
    .catch(() =>
      cb(
        create_error(
          403,
          "Insufficient permissions. The caller's permissions must be a superset of the execution permissions of the function."
        )
      )
    );
}
...
function compute_build_plan_core(ctx, cb) {
  ...
  if (hashes.functionPermissions !== existingHashes.functionPermissions) {
    // Only permissions have changed, no need to touch the function.
    return set_build('metadata_update');
  }

  if (ctx.options.internal.authentication !== ctx.options.internal.existing.internal.authentication) {
    // Only authentication mode has changed, no need to touch the function.
    return set_build('metadata_update');
  }

  if (hashes.authorizations !== existingHashes.authorizations) {
    // Only authorizations have changed, no need to touch the function.
    return set_build('metadata_update');
  }
...
function resolve_dependencies_from_unpkg(ctx, cb) {
  ...
  function resolve_one_dependency(ctx, name, cb) {
    var url = `https://unpkg.com/${name}@${ctx.options.internal.pending_dependencies[name]}/package.json`;
    return Superagent.get(url)
      ...
      .then((res) => {
        ...
        return cb(
          create_error(
            400,
            `Unable to fully resolve module version for ${name}@${ctx.options.internal.pending_dependencies[name]} from ${url}`
          )
        );
      })
      .catch((err) => {
        return cb(
          create_error(
            400,
            `Unable to fully resolve module version for ${name}@${ctx.options.internal.pending_dependencies[name]} from ${url}: ${err.message}`
          )
        );
      });
  }
