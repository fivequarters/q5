...
function put_function_core(options, registry, auth, cb) {
  ...
  return Async.series(
    [
      ...
      (cb) => {
        // Make sure the function gets enabled appropriately, even if nothing else has changed about the
        // function spec to cause a build.
        if (ctx.options.internal.build_plan === 'none') {
          return finalize_function_status(ctx.options, cb);
        }
        return cb();
      },
    ],
...
function compute_build_plan(ctx, cb) {
  ...
  ctx.options.internal.hashes.permissions = hash_object({ permissions: ctx.options.permissions });
...
function resolve_settings(ctx, cb) {
  ...
  return Async.each(
    ctx.options.permissions.allow,
    async (entry) => ctx.authorization.agent.ensureAuthorized(entry.action, entry.resource),
    (e) => {
      if (e) {
        return cb(create_error(403, 'Insufficient permissions'));
      }
      return cb();
    }
  );
...
function compute_build_plan_core(ctx, cb) {
  ...
  if (hashes.permissions !== existingHashes.permissions) {
