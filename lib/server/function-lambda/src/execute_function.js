...
const invoke_handler = (req, res, next, e, r, m) => {
  console.log('[lambda-call] - calling:', req);
  ...
  if (!r) {
    res.status(200);
    console.log('[lambda-call] - no response');
    return res.end();
  }

  // Process the result from the invocation.
  res.status(r.status || 200);
  console.log('[lambda-call] - result status: ', r.status || 200);
  ...
  if (r.bodyEncoding && typeof r.body === 'string') {
    ...
    console.log('[lambda-call] - result body: ', r.body);
