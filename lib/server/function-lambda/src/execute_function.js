...
const invoke_handler = (req, res, next, e, r, m) => {
  ...
  /* XXX are we exposing logs to the response? */
  if (m.log) {
    let logsSerialized = m.log.join('\n');
    if (logsSerialized.length > 4096) {
      logsSerialized = logsSerialized.substring(logsSerialized.length - 4096);
    }
    res.set('x-fx-logs', Buffer.from(logsSerialized).toString('base64'));
    res.log = m.log;
  }
