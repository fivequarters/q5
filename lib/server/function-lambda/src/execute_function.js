  ...
  return module.exports.core(
    ...
    (e, r) => {
      if (e) {
        ...
        } else if (e.code === 'ConcurrentInvocationLimitExceeded') {
          console.log(
            `ERROR: Account Lambda concurrency limit exceeded when running function ${req.params.subscriptionId}/${
              req.params.boundaryId
            }/${req.params.functionId}`
          );
          return next(create_error(503));
        } else if (e.code === 'ReservedFunctionConcurrentInvocationLimitExceeded') {
          console.log(
            `ERROR: Reserved Lambda function concurrency limit exceeded when running function ${
              req.params.subscriptionId
            }/${req.params.boundaryId}/${req.params.functionId}`
          );
          return next(create_error(503));
        } else {
          console.log(
