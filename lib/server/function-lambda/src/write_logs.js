...
const { publishLogs } = require('@5qtrs/runtime-common');
...
module.exports = function write_logs(params, logEntries, cb) {
  dynamodb_write_logs(params, [...logEntries], (e) => {
    if (e) {
      return cb(e);
    }
    loki_write_console_logs(params, [...logEntries], cb);
  });
};

const loki_write_console_logs = async (params, logEntries, cb) => {
  try {
    await publishLogs(params, logEntries);
  } catch (error) {
...
function dynamodb_write_logs(params, logEntries, cb) {
