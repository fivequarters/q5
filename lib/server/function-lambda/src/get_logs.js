...
export function get_logs(req, res, next) {
  ...
  Assert.ok(req.params.boundaryId.match(Constants.valid_boundary_name), 'boundary name must be valid');
  if (req.params.functionId) {
    Assert.ok(req.params.functionId.match(Constants.valid_function_name), 'function name must be valid');
  }
        ...
        dynamo.deleteItem(
          ...
          (e) => {
            if (e && e.code !== 'ConditionalCheckFailedException') {
              console.error(
                'ERROR: Unable to delete real-time logging connection information from Dynamo:',
                e.code,
                e.message
              );
            }
