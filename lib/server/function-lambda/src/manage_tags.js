...
export function delete_function_tags(options, cb) {
  console.log(`deleting tags: ${JSON.stringify(options)}`);
...
export function search_function_tags(options, key, value, next, limit, cb) {
  ...
  console.log(`search ${params.ExpressionAttributeValues[':sortKeyPrefix'].S}`);
  ...
  dynamo.query(params, (e, d) => {
    ...
    return cb(
      null,
      d.Items.map((i) => {
        return { boundaryId: i.boundaryId.S, functionId: i.functionId.S };
      }),
      d.LastEvaluatedKey
  ...
  setTimeout(() => {
    ...
    dynamo.batchWriteItem({ RequestItems: { [keyValueTableName]: items } }, (e, d) => {
      ...
      if (Object.keys(d.UnprocessedItems).length > 0) {
        ...
        console.log(`batch continue: ${overflow.length} items`);
