      ...
      d.Items.map((i) => {
        try {
          return {
            ...
            location: Common.get_function_location(options.req, options.subscriptionId, i.boundaryId.S, i.functionId.S),
          };
        } catch (e) {
...
function execute_dynamo_batch_write(items, cb, backoff = 0) {
  ...
  setTimeout(
    ...
    backoff > 0 ? Math.pow(DYNAMO_BACKOFF_DELAY, backoff) : 0
  );
}
...
export function convert_spec_to_tags(spec) {
  ...
  if (spec.internal && spec.internal.versions) {
    for (let t in spec.internal.versions) {
...
function get_dynamo_delete_request(category, key) {
  return [
    {
      DeleteRequest: {
        Key: {
