...
export function create_function_tags(options, spec, cb) {
  console.log(`create_function_tags ${JSON.stringify(options)} ${JSON.stringify(spec)}`);
...
export function search_function_tags(options, criteria, cb) {
  let continuationToken = undefined;
  let params = {
    TableName: keyValueTableName,
    ProjectionExpression: 'boundaryId, functionId',
    Limit: 100,
    ExpressionAttributeValues: {
      ':searchCategory': { S: options.boundaryId ? TAG_CATEGORY_BOUNDARY : TAG_CATEGORY_SUBSCRIPTION },
      ':sortKey': { S: 'key' },
      ':sortKeyPrefix': {
        S: options.boundaryId ? get_bound_sort_key(options, key, value) : get_sub_sort_key(options, key, value),
      },
    },
    KeyConditionExpression: `category = :searchCategory AND begins_with(:sortKey, :sortKeyPrefix)`,
    ExclusiveStartKey: continuationToken, // Likely will need to be extracted and re-packed
  };

  dynamodb.query(params, (e, d) => {
    if (e) return cb(e);
    // Do some kind of mapping of data.Items
    console.log(d);
    return cb(null, d);
  });
}
...
function execute_dynamo_batch_write(items, cb, backoff = 0) {
  ...
  console.log(
    `execute_dynamo_batch_write ${JSON.stringify(
      items.map((i) => (i.DeleteRequest && 'D/' + i.DeleteRequest.Key.key.S) || 'P/' + i.PutRequest.Item.key.S)
    )}, ${backoff}`
  );
  ...
  setTimeout(() => {
    ...
    dynamo.batchWriteItem({ RequestItems: { [keyValueTableName]: items } }, (e, d) => {
      ...
      if (Object.keys(d.UnprocessedItems).length > 0) {
        ...
        return execute_dynamo_batch_write(d.UnprocessedItems.push(...overflow), cb, backoff + 1);
...
function scan_dynamo_function_tags(options, cb, results = [], lastEvaluatedKey = undefined) {
  console.log(`Scanning ${JSON.stringify(options)}`);
  return dynamo.scan(
    {
      TableName: keyValueTableName,
  ...
  return [
    {
      PutRequest: {
        Item: {
          ...
          ...get_dynamo_promoted_attributes(options, key, value),
        },
      },
    },
    {
