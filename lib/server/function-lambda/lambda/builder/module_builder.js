...
function zip_package(event, cb) {
  ...
  return zipdir(Path.join(event.dest_path, 'node_modules', event.name), { saveTo: event.package_file }, (e) => cb(e));
}

function run_npm_install(event, cb) {
  console.log('XXX', event.npmrc);
  Fs.writeFileSync('.npmrc', event.npmrc);
  exec(
    'npm install --verbose --production --global-style ' + event.name + '@' + event.version,
    {
      cwd: event.dest_path,
