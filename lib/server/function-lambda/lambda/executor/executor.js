...
[
  ...
  const createSpan = (args) => {
    ...
    // Requests to Fusebit infrastructure will be logged on receipt.
    if ((host || hostname).endsWith('fusebit.io') || (host || hostname).endsWith('fivequarters.io')) {
      return undefined;
    }
...
exports.execute = function execute(event, context, cb) {
  ...
  const errorHandler = (error) => {
    ...
    return { status: 500, body: errorToObj(error), spans, logs: requestLogs, ids: requestIds };
  };
...
function hookupConsole(console, method) {
  ...
  // Undocumented interface to add related objects to this invocation.
  console['related'] = function (ids) {
    try {
      requestIds.push(...ids);
    } catch (err) {
      throw new Error(`Unable to parse a supplied list of related ids: ${err}`);
    }
  };
}
