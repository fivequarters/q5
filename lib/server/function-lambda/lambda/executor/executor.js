        ...
        const req = client.request(scheduleUrl, httpOptions, (res) => {
          ...
          res.on('end', () => {
            if (res.statusCode !== 202) {
              rejectWithError(new Error(`Unexpected response to the task scheduling request`));
            }
            try {
              body = JSON.parse(body);
            } catch (e) {
              return rejectWithError(new Error(`Malformed response to the task scheduling request`));
            }
