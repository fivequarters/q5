...
const errorToObj = (error) => ({ code: error.code, message: error.message, stack: error.stack.split('\n') });
...
[
  ...
  const normalizeOptions = (args, defaultMethod) => {
    ...
    if (typeof args[0] === 'object') {
      options = JSON.parse(JSON.stringify(args[0]));
  ...
  if (handler.constructor.name === 'AsyncFunction') {
    ...
    return handler(event)
      .then((result) => {
        ...
        console.log._fusebit(`XXX XXX requestLogs length: ${requestLogs.length}`);
