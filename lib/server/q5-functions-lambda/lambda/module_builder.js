...
exports.build = function build(event, context, cb) {
    ...
    return series([
        function (cb) { run_npm_install(event, cb); },
...
function zip_package(event, cb) {
    exec('python ' + Path.join(__dirname, 'zip.py'), {
        cwd: Path.join(event.dest_path, 'node_modules', event.name)
    }, function (error, stdout, stderr) {
        if (error) return cb(error);
        event.package_file = Path.join(event.dest_path, 'node_modules', event.name, 'package.zip');
        console.log(stdout);
...
function run_npm_install(event, cb) {
    exec('npm install --production --global-style ' + event.name + '@' + event.version, {
        cwd: event.dest_path,
