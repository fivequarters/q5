...
module.exports = function lambda_get_function_build(req, res, next) {
    return module.exports.core(req.params, (e, r) => {
        ...
        switch (r.status) {
            case 'success': res.status(200); break;
            case 'pending': 
            case 'building': res.status(201); break;
            case 'failure': 
            default: res.status(410); break;
        }
...
module.exports.core = function lambda_get_function_build_core(options, cb) {
    ...
    Assert.equal(typeof(options.build_id), 'string', 'build_id must be provided');
