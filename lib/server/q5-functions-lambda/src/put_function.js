...
module.exports = function lambda_put_function(req, res, next) {
    ...
    if (package_json && package_json.engines && typeof package_json.engines.node === 'string') {
        try {
            ...
            return next(create_error(400, `Unable to find Node.js runtime version matching the "${package_json.engines.node}" requirement. Supported versions are: ${runtime_versions.join(', ')}.`));
        }
    }
    else {
    ...
    return module.exports.core(options, (e, r) => {
        if (e) return next(create_error(500, `Error creating function: ${e.message}.`));
        if (r.synchronous) {
            if (r.data) {
...
module.exports.core = function lambda_put_function_core(options, cb) {
    ...
    Assert.equal(options.nodejs.files['executor.js'], undefined, 'the executor.js file must NOT be specified');
    for (var file_name in options.nodejs.files) 
        Assert.ok(typeof options.nodejs.files[file_name] === 'string' || typeof options.nodejs.files[file_name] === 'object', `file ${file_name} must be a string or object`);
    Assert.ok(!options.lambda || typeof(options.lambda) === 'object', 'lambda, if present, must be an object');
    Assert.ok(!options.environment || typeof(options.environment) === 'object', 'environment, if present, must be an object');
    ...
    let initial_build_status = {
        ...
        name: options.name,
        build_id: Math.floor(Math.random() * 999999999).toString(32),
        transitions: {
            pending: new Date().toString(),
        }
    };

    options.build_id = initial_build_status.build_id;
...
// (cb) => { console.log('BUILD PLAN', ctx.options.internal); cb(); }, // KILL
        (cb) => save_function_build_status(ctx, cb),
        (cb) => save_function_build_request(ctx, cb),
        (cb) => run_build(ctx, (e,d) => { ctx.result = d; cb(e); }),
    ], e => cb(e, ctx.result));
...
function compute_build_plan(ctx, cb) {
    ...
    return Async.series([
        (cb) => determine_dependencies_available(ctx, cb),
        (cb) => compute_build_plan_core(ctx, cb)
    ], cb);

    function hash_object(o) {
        var hash = Crypto.createHash('md5');
        Object.keys(o).sort().forEach(prop => {
            hash.update(prop);
            hash.update(':');
...
function compute_build_plan_core(ctx, cb) {
    ...
    if (ctx.options.internal.new_metadata.dependencies_hash !== ctx.options.internal.existing_metadata.dependencies_hash) {
    ...
    if (ctx.options.internal.new_metadata.configuration_hash !== ctx.options.internal.existing_metadata.configuration_hash) {
...
function determine_dependencies_available(ctx, cb) {
    ...
    if (ctx.options.internal.new_metadata.runtime_hash === ctx.options.internal.existing_metadata.runtime_hash
        ...
        return Async.series([
            ...
            (cb) => check_dependencies_present_in_s3(ctx, cb),
        ], cb);
    }
}
    ...
    function resolve_one_dependency(ctx, name, cb) {
        ...
        return Superagent.get(url)
            ...
            .then(res => {
                ...
                return cb(new Error(`Unable to fully resolve module version for ${name}@${ctx.options.internal.dependencies[name]} from ${url}`));
            })
            .catch(err => {
                return cb(new Error(`Unable to fully resolve module version for ${name}@${ctx.options.internal.dependencies[name]} from ${url}: ${err.message}`));
            });
    }
}
    ...
    function check_one_dependency_present_in_s3(ctx, name, cb) {
        return Common.S3.getObject({
            ...
            if (e) {
                ...
                return cb(new Error(`Unable to determine availability of module ${name}@${ctx.options.internal.resolved_dependencies[name]} for runtime ${ctx.options.lambda.runtime}: ${e.error}`));
            }
            try {
            ...
            if (d.Body.backoff && d.Body.backoff > Date.now()) {
                ...
                return cb(new Error(`Module ${name}@${ctx.options.internal.resolved_dependencies[name]} for runtime ${ctx.options.lambda.runtime} failed to build previously and another attempt is delayed until ${new Date(+d.Metadata.backoff).toUTCString()}. Last error: ${d.Metadata.error}`));
            }
...
function run_build(ctx, cb) {
    ...
    if (ctx.options.internal.build_plan === 'full_build') {
        ...
        return Worker(
            ctx.build_status, 
            (e, d) => e ? cb(e) : cb(null, { synchronous: true, data: d })
        );
    }
}
