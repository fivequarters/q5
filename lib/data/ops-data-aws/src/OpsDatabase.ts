...
export async function getDatabaseCredentials(
  ...
  if (!data.SecretList || data.SecretList.length !== 1) {
    throw new Error(
      `Cannot find a unique secret to access Aurora cluster. Expected 1 matching secret, found ${data.SecretList && data.SecretList.length > 0 ? data.SecretList.length : ". Delete the Aurora cluster and try again."}`
    );
  }
