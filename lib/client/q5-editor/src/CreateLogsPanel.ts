...
export function createLogsPanel (element: HTMLElement, workspace: Workspace, options?: ILogsPanelOptions) {
    ...
    workspace.on(Events.Events.BuildStarted, function (e: Events.BuildStartedEvent) {
        append(`BUILD: starting build of ${workspace.functionSpecification.boundary}/${workspace.functionSpecification.name}...`);
    });
    ...
    workspace.on(Events.Events.BuildFinished, <EventListener> function (e: Events.BuildFinishedEvent) {
        if (e.status.build_id)
            append(`BUILD ${e.status.build_id}: progress ${Math.floor((e.status.progress || 0) * 100)}% (${e.status.status}) ${e.status.url || JSON.stringify(e.status.error, null, 2)}`);
        else
    ...
    workspace.on(Events.Events.RunnerFinished, function (e: Events.RunnerFinishedEvent) {
        if (e.error) {
            ...
            let lines: string[] = [ `RUN: received response HTTP ${e.response ? e.response.statusCode : 'N/A'} ${e.response ? e.response.statusMessage : ''}` ];
            if (e.response) {
