...
import { Events, DirtyStateChangedEvent } from './Events';
...
export function createActionPanel (element: HTMLElement, workspace: Workspace, server: Server, options?: IActionPanelOptions) {
    ...
    let lines: string[] = [`<div class="q5-action-wrapper">
    <div>`];
    if (opts.enableClose) {
        lines.push(`        <button id="${closeId}" class="q5-action-btn"><i class="fa fa-window-close"></i> Close</button>`);
    }
...
`);
    ...
    $save.click(function (e) {
        ...
        server.buildFunction(workspace)
            .then(_ => {
