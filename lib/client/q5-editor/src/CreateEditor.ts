...
export function createEditor (element: HTMLElement, workspace: Workspace, server: Server, options?: Options.IEditorOptions) {
    ...
    if (opts.editorPanel !== false) {
        createEditorPanel(<HTMLElement> document.getElementById(editorId), workspace, <Options.IEditorPanelOptions> opts.editorPanel);
    }
    if (opts.statusPanel !== false) {
        createStatusPanel(<HTMLElement> document.getElementById(statusId), workspace, <Options.IStatusPanelOptions> opts.statusPanel);
    }
    if (opts.navigationPanel !== false) {
        createNavigationPanel(<HTMLElement> document.getElementById(navId), workspace, <Options.INavigationPanelOptions> opts.navigationPanel);
    }
    if (opts.actionPanel !== false) {
        createActionPanel(<HTMLElement> document.getElementById(actionId), workspace, server, <Options.IActionPanelOptions> opts.actionPanel);
    }
    if (opts.logsPanel !== false) {
        createLogsPanel(<HTMLElement> document.getElementById(logsId), workspace, <Options.ILogsPanelOptions> opts.logsPanel);
    }

    workspace.on(Events.Events.NavStateChanged, <EventListener> function (e: Events.NavStateChangedEvent) {
        if (e.newState) {
            $nav.show();
            $navSplitter.show();
        }
    ...
    workspace.on(Events.Events.LogsStateChanged, <EventListener> function (e: Events.LogsStateChangedEvent) {
        if (e.newState) {
            $logs.show();
            $logsSplitter.show();
        }
    ...
    workspace.on(Events.Events.FullScreenChanged, <EventListener> function (e: Events.FullScreenChangedEvent) {
        if (e.newState) {
            // expand
