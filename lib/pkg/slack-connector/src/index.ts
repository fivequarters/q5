...
router.on('startup', async ({ mgr, cfg }: Types.IOnStartup, next: Types.Next) => {
  cfg.configuration.tokenUrl = TOKEN_URL;
  cfg.configuration.authorizationUrl = AUTHORIZATION_URL;
  return next();
});

router.get('/api/configure', connector.middleware.authorizeUser('connector:put'), async (ctx: Types.Context) => {
  ctx.body = {
