...
const createViewRow = (profile: IFusebitProfile) => (
  ...
  name: `${[dataRow.firstName, dataRow.lastName, dataRow.displayName]
    .join(" ")
    .trim() || "N/A"}${
...
const getActionRowSpan = (row: ViewRow, tableRow: number): number => {
  if (
    tableRow > 0 &&
    row.permissions[tableRow - 1].action === row.permissions[tableRow].action
  ) {
    return 0;
  }
  let rowSpan = 1;
  while (
    ...
    row.permissions[tableRow + rowSpan].action ===
      row.permissions[tableRow].action
  ) {
    rowSpan++;
  }
  return rowSpan;
...
function ResourceAccess({
  ...
  const headCells: HeadCell<ViewRow>[] = [
    ...
    },
    {
      ...
      getRowSpan: (row, tableRow) =>
        tableRow === 0 ? row.permissions.length || 1 : 0
    },
    {
      id: "action",
      align: "left",
      ...
      render: (row, tableRow) =>
        ...
        row.permissions[tableRow - 1].action ===
          row.permissions[tableRow].action ? (
          undefined
        ) : (
      ...
      render: (row, tableRow) =>
        (row.permissions[tableRow] && row.permissions[tableRow].resource && (
          <FunctionResourceCrumb
            options={tryTokenizeResource(row.permissions[tableRow].resource)}
          />
        )) ||
        "N/A"
    }
          ...
          for (var resourceComponent in resourceFilter) {
            if (
              ...
              resource[resourceComponent] !==
                (resourceFilter as any)[resourceComponent]
            ) {
              return;
            }
          }
