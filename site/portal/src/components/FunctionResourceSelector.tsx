...
function FunctionResourceSelector({ resource, disabled, variant, onResourceChange }: any) {
  const classes = useStyles();
  const { profile } = useProfile();
  const subscriptionSelected = resource.parts.subscriptionId && resource.parts.subscriptionId !== '*';
  ...
  const handleSubscriptionIdChange = (subscriptionId: string) => {
    ...
    if (trimmed.length > 0 && trimmed !== '*' && (trimmed.indexOf('sub-') !== 0 || trimmed.length !== 20)) {
      parts.subscriptionIdError = "Subscription ID must have 20 characters and start with 'sub-'.";
    } else {
  ...
  const handleBoundaryIdChange = (value: string) => {
    ...
    if (trimmed.length > 0 && !trimmed.match(/^[a-z0-9-]{1,63}$/)) {
      parts.boundaryIdError = 'Boundary ID must must have between 1 and 63 lowercase letters, digits, and dashes.';
    } else {
      delete parts.boundaryIdError;
    }
    onResourceChange &&
  ...
  const handleFunctionIdChange = (value: string) => {
    ...
    if (trimmed.length > 0 && !trimmed.match(/^[a-z0-9-]{1,64}$/)) {
      parts.functionIdError = 'Function ID must must have between 1 and 64 lowercase letters, digits, and dashes.';
    } else {
      delete parts.functionIdError;
    }
    onResourceChange &&
  ...
  const hasError = (parts: any) => !!(parts.subscriptionIdError || parts.boundaryIdError || parts.functionIdError);
  ...
  return (
    <React.Fragment>
      <SubscriptionSelector
        ...
        disabled={!!disabled}
      />
      <BoundariesProvider subscriptionId={subscriptionSelected ? resource.parts.subscriptionId : undefined}>
        <BoundarySelector
          subscriptionId={subscriptionSelected ? resource.parts.subscriptionId : undefined}
        ...
        <FunctionSelector
          subscriptionId={
            subscriptionSelected && !resource.parts.boundaryIdError ? resource.parts.subscriptionId : undefined
          }
          boundaryId={resource.parts.boundaryIdError ? undefined : resource.parts.boundaryId}
          functionId={resource.parts.functionId}
          disabled={
            !!disabled || !!!(subscriptionSelected && resource.parts.boundaryId && !resource.parts.boundaryIdError)
          }
          fullWidth
          variant={variant || 'filled'}
