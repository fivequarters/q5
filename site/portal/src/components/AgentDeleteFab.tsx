...
function AgentDeleteFab() {
  ...
  const handleDeleteDialogClose = async (confirmed: boolean) => {
    ...
    if (confirmed) {
      try {
        agent.isUser ? await deleteUsers(profile, [agent.agentId]) : await deleteClients(profile, [agent.agentId]);
    ...
    <React.Fragment>
      ...
      {confirmationOpen && (
        <ConfirmationDialog
          ...
          content={
            <div>
              <DialogContentText>Deleting a {noun} cannot be undone.</DialogContentText>
              {profile.me && profile.me.id === agent.agentId && (
                <React.Fragment>
                  <Typography color="primary">WARNING</Typography>
                  <DialogContentText>
                    The user you are about to delete is you. If you continue, you will loose access to the system.
                  </DialogContentText>
                </React.Fragment>
              )}
            </div>
