...
import { getClient, getUser, newClient, newUser, normalizeAgent, updateClient, updateUser } from '../lib/Fusebit';
...
const AgentStateContext = React.createContext<AgentState | undefined>(undefined);
const AgentSetStateContext = React.createContext<AgentSetState | undefined>(undefined);
...
function AgentProvider({ children, agentId, isUser }: AgentProviderProps) {
      ...
      (async () => {
        ...
        try {
          ...
          if (data.status === 'loading') {
            agent = isUser ? await getUser(profile, agentId) : await getClient(profile, agentId);
          ...
          if (!cancelled) {
            const error = data.formatError
              ? data.formatError(e)
              : new FusebitError(`Error ${data.status} ${isUser ? 'user' : 'client'} ${data.agentId}`, {
                  details:
                    (e.status || e.statusCode) === 403
                      ? `You are not authorized to access the ${isUser ? 'user' : 'client'} information.`
  ...
  return (
    <AgentStateContext.Provider value={data}>
      <AgentSetStateContext.Provider value={setData}>{children}</AgentSetStateContext.Provider>
...
function modifyAgent(state: AgentState, setState: AgentSetState, newAgent: Client | User) {
  ...
  setState({
    ...state,
    dirty: JSON.stringify(normalizeAgent(state.existing)) !== JSON.stringify(normalizeAgent(newAgent)),
    modified: newAgent,
  });
}
...
function formatAgent(agent: AgentState) {
  if (agent.status === 'updating' || agent.status === 'ready') {
    let name = [];
    if ((agent.modified as User).firstName) name.push((agent.modified as User).firstName);
    if ((agent.modified as User).lastName) name.push((agent.modified as User).lastName);
    if ((agent.modified as Client).displayName) name.push((agent.modified as Client).displayName);
    if (name.length > 0) {
      return name.join(' ');
    }
  }
...
export { AgentProvider, useAgent, modifyAgent, saveAgent, reloadAgent, formatAgent, useAgentMaybe };
