...
const createAccessView = (access: any) => ((access && access.allow) || []).map(createViewRow);

function AgentPermissions() {
  const [agent, setAgent] = useAgent();
  const { profile } = useProfile();
  const [editPermissionsDialogOpen, setEditPermissionsDialogOpen] = React.useState(false);
      ...
      <div>
        ...
        {agent.agentId === (profile.me && profile.me.id) && (
          <React.Fragment>
            ...
            <DialogContentText>
              You are deleting permissions for the currently logged in user. If you continue, you may loose access to
              the portal.
            </DialogContentText>
          </React.Fragment>
        )}
  ...
  if (agent.status === 'loading') {
    return <ProgressView />;
  } else if (agent.status === 'ready' || agent.status === 'updating' || agent.status === 'error') {
    return (
      <React.Fragment>
        {(agent.status === 'ready' || agent.status === 'updating') && (
          <ExplorerTable<ViewRow>
            ...
            title="Permissions"
            noDataBody={
              <Typography>The {agent.isUser ? 'user' : 'client'} has no permissions in the system.</Typography>
            }
            actions={
              <React.Fragment>
                <ActionButton disabled={agent.status !== 'ready'} onClick={() => setAddPermissionDialogOpen(true)}>
                  Add&nbsp;permission
                </ActionButton>
                <ActionButton disabled={agent.status !== 'ready'} onClick={() => setAddPermissionSetOpen(true)}>
                  Add&nbsp;permission&nbsp;set
                </ActionButton>
                <ActionButton disabled={agent.status !== 'ready'} onClick={() => setEditPermissionsDialogOpen(true)}>
                  Edit&nbsp;as&nbsp;JSON
                </ActionButton>
              </React.Fragment>
            }
        ...
        {editPermissionsDialogOpen && <EditPermissionsAsJsonDialog onClose={handleSaveJsonPermissions} />}
        {addPermissionDialogOpen && <AddPermissionDialog onClose={handleAddPermission} />}
