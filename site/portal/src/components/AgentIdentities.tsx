  ...
  if (agent.status === 'ready' || agent.status === 'updating') {
    return (
      <div className={classes.identityContainer}>
        {(!agent.modified.identities || agent.modified.identities.length === 0) && (
          <div>
            <Typography>
              The {agent.isUser ? 'user' : 'client'} has no identities. You must add at least one identity before the{' '}
              {agent.isUser ? 'user' : 'client'} can authenticate and access the system.
            </Typography>
          </div>
        )}
              ...
              actions={
                <React.Fragment>
                  ...
                  <Button
                    ...
                    to={`../../issuers/${encodeURIComponent(identity.issuerId)}/properties`}
                    disabled={agent.status !== 'ready'}
                  >
                    View issuer
                  </Button>
