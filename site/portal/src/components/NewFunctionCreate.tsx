...
function NewFunctionCreate({
  ...
  const data = query.get("data")
    ? JSON.parse(Buffer.from(query.get("data") as string, "base64").toString())
    : {};
      ...
      (async () => {
        try {
          ...
          await Superagent.post(
            appendUrlSegment(template.managerUrl, "install")
          )
            .set(
              "Authorization",
              `Bearer ${(await ensureAccessToken(profile)).access_token}`
            )
            .send({
              baseUrl,
              accountId,
  ...
  const doneUrl =
    state.status === "success"
      ? `/accounts/${data.accountId}/subscriptions/${data.subscriptionId}/boundaries/${data.boundaryId}/functions/${data.functionId}/overview`
  ...
  return (
    <Dialog open={true} onClose={handleClose} maxWidth="sm" fullWidth>
      {(catalog.status === "loading" ||
        (catalog.status === "ready" && state.status === "installing")) && (
        <React.Fragment>
      ...
      {(catalog.status === "error" || state.status === "error") && (
        <React.Fragment>
          ...
          <DialogContent>
            <DialogContentText>
              Error creating function <strong>{data.functionId}</strong> in{" "}
              <strong>{data.boundaryId}</strong> boundary using{" "}
              <strong>{data.templateName}</strong> template.
            </DialogContentText>
            <PortalError
              error={
                (catalog.status === "error" && catalog.error) || state.error
              }
            />
          </DialogContent>
        </React.Fragment>
      ...
      {state.status === "success" && (
        <React.Fragment>
          ...
          <DialogContent>
            <DialogContentText>
              Function <strong>{data.functionId}</strong> created successfuly in{" "}
              <strong>{data.boundaryId}</strong> boundary using{" "}
              <strong>{data.templateName}</strong> template.
            </DialogContentText>
          </DialogContent>
      ...
      <DialogActions>
        <Button
          ...
          color="primary"
          disabled={
            state.status === "installing" || catalog.status === "loading"
          }
        >
          Done
        </Button>
