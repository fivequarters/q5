...
function AddOAuthImplicitIdentityFlow({ options, onDone, variant }: any) {
  ...
  React.useEffect(() => {
    ...
    if (!initToken) {
      (async () => {
        ...
        try {
          data = {
            token: await getInitToken(augmentedProfile, agent.agentId, 'oauth', true),
          };
        } catch (e) {
          data = {
  ...
  if (!initToken) {
    ...
    return (
      <DialogContent>
        <DialogContentText>
          Share the following invitation link with the user over a secure channel. The one-time initialization token
          contained in the link is valid for eight hours.
        </DialogContentText>
        <TextField
          margin="dense"
          ...
          InputProps={{
            endAdornment: (
              <InputAdornment position="end">
                <IconButton onClick={() => navigator.clipboard.writeText(inviteLink)} color="inherit">
