...
function AddIdentityDialog({ onClose }: any) {
  ...
  const handleIssuerIdChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (trimmed.length === 0) {
      addIdentity.issuerIdError = 'Required. The value of the iss claim in the JWT access token';
    } else {
  ...
  const handleSubjectChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (trimmed.length === 0) {
      addIdentity.subjectError = 'Required. The value of the sub claim in the JWT access token';
    } else {
    ...
    >
      ...
      <DialogContent>
        <TextField
          ...
          label="Issuer ID"
          variant="filled"
          helperText={addIdentity.issuerIdError || 'The value of the iss claim in the JWT access token'}
          fullWidth
        ...
        <TextField
          ...
          label="Subject"
          variant="filled"
          helperText={addIdentity.subjectError || 'The value of the sub claim in the JWT access token'}
      ...
      <DialogActions>
        ...
        <Button onClick={handleSubmit} color="secondary" disabled={hasError()} variant="contained">
          Add
        </Button>
      </DialogActions>
    </Dialog>
