...
function FunctionActionFab() {
  ...
  React.useEffect(() => {
    if (confirmDelete.status === 'initiating') {
      (async () => {
        ...
        try {
          await deleteFunctions(profile, func.subscriptionId, func.boundaryId, [func.functionId]);
        } catch (e) {
          setBoundaries({
            status: 'error',
  ...
  return (
    <div className={classes.root}>
      ...
      {editMetadataOpen && <EditMetadataDialog onClose={() => setEditMetadataOpen(false)} />}
      <ConfirmationDialog
        open={confirmDelete.status !== 'closed'}
        title={confirmDelete.status === 'open' ? 'Confirm delete' : 'Deleting...'}
        content={
          confirmDelete.status === 'open' ? (
            <DialogContentText>
              Delete function <strong>{func.functionId}</strong> in boundary <strong>{func.boundaryId}</strong>?
            </DialogContentText>
          ) : (
            <LinearProgress />
          )
