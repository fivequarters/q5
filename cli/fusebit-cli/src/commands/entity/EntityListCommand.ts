...
const createCommand = (options: IEntityCommandOptions) => ({
  ...
  description: `Lists ${options.plural} of a specific ${options.parentEntityUrlSegment}.`,
  arguments: [
    {
...
export class EntityListCommand extends Command {
  ...
  protected async onExecute(input: IExecuteInput): Promise<number> {
    ...
    if (output === 'json') {
      const result = await entityService.listEntities(parentEntityId, tags, options);
